...
public class Dashboard extends JFrame implements ActionListener {
    ...
    public Dashboard(String username) {
        ...
        // NOTE: The WEST Panel (menuPanel) is REMOVED/NOT ADDED
        ...
        imagePanel.setBackground(MAIN_CONTENT_LIGHT_GREY); // Set image panel background

        // NOTE: Keeping the image path 
        ...
        // Keeping it white/light grey for contrast against the image
    ...
    // Helper method to create the Header Panel (Modified to include buttons)
    private JPanel createHeaderPanel() {
        JPanel p1 = new JPanel();
        // Set to Black
        p1.setBackground(HEADER_BLACK); 
        // Use a BoxLayout for vertical stacking within the NORTH region
    ...
    // Helper method to create the Menu Bar (uses FlowLayout for horizontal buttons)
    private JPanel createMenuBar() {
        JPanel menuBar = new JPanel();
        // Set background to a darker color for contrast with the black header bar above it
        menuBar.setBackground(BUTTON_DARK_GREY); 
        // Use FlowLayout for horizontal button arrangement with left alignment and padding
        menuBar.setLayout(new FlowLayout(FlowLayout.LEFT, 10, 5)); 

        int buttonIndex = 0;
        for (String label : buttonLabels) {
            
            // NOTE: Separator logic is removed as horizontal separators look strange here.
            
            JButton button = new JButton(label);
            // Set button background to a slight variant of the header color
            button.setBackground(HEADER_BLACK); 
            button.setForeground(Color.WHITE);
            button.setFont(new Font("Tahoma", Font.PLAIN, 14)); // Smaller font for horizontal menu
            button.setMargin(new Insets(5, 10, 5, 10)); // Add padding inside button
    ...
    // The actionPerformed method remains unchanged as it uses button labels
    @Override
